...
def meta_tail(
    ...
    *,
    ...
    AussieBB tools do no scope-filtering/pagination at the response layer, so
    ``matched_total == returned`` and ``filtered_by`` is empty. The diagnostic
    write tool passes ``target_id``/``rows_affected``. ``latency_ms`` derives
