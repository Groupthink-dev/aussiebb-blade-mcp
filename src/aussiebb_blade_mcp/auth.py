...
Bearer token auth for the manual loopback HTTP path. Set ``ABB_MCP_API_TOKEN``
...
the http transport unless ``ABB_MCP_API_TOKEN`` is set and the host is loopback
(per the blade-mcp transport policy — DD-242 / access-policy). That gate, not
this pass-through, is what guarantees the http surface is always authenticated.
"""
...
def get_bearer_token() -> str | None:
    ...
    token = os.environ.get("ABB_MCP_API_TOKEN", "").strip()
...
class BearerAuthMiddleware:
    ...
    When ``ABB_MCP_API_TOKEN`` is set, every request must carry a matching
